...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/16 15:47:13 by agras             #+#    #+#             */
/*   Updated: 2022/08/07 14:29:19 by agras            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	isolate_right(t_prog *prog, t_token *token, char *line)
{
	...
	if (*line != '\0')
	{
		...
		if (token_add_right_child(&token, token_new(data, OUTFILE,
					OUTFILE, token->mask)) == FATAL_ERR)
			ft_exit(prog, NULL, "Memory allocation failded\n");
